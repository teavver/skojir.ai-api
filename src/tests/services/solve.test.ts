import { testUser, setupTests, teardownTests, gcfStatusURL, solveURL } from "../_setup.js"
...
describe("[SERVICES] Test backend service", function () {
    ...
    this.timeout(40000) // Higher timeout for GCF cold starts
    ...
    it("Should reject /solve req without the Bearer token", async () => {
        const data = {
            threshold: 0.25, img: imgData, outputFormat: "standard"
        }
        const req = () => axios.post(solveURL, data)
        const res = await testAxiosRequest(MODULE, req)
    ...
    it("Use /solve with valid auth token", async () => {
        const data = {
            threshold: 0.25, img: imgData, outputFormat: "minimal"
        }
